...
public class RegistrationFilter implements Filter {
    ...
    private void doBeforeProcessing(ServletRequest request, ServletResponse response)
        ...
        //if (debug) log("RegistrationFilter:DoBeforeProcessing");
    ...
    private void doAfterProcessing(ServletRequest request, ServletResponse response)
            throws IOException, ServletException {
        //if (debug) log("RegistrationFilter:DoAfterProcessing");
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        try {
            ...
            //
            if (txtEmail == null || txtEmail.trim().isEmpty() || txtEmail.length() < 6 || txtEmail.length() > 50) {
                error.setEmailLengthErr("Username Length must be between 6-50 chars");
                valid = false;
            }
            //
            if (txtName == null || txtName.trim().isEmpty() || txtName.length() > 100) {
                error.setNameLengthErr("Name must not be empty and under 100 chars");
                valid = false;
            }
            //
            if (password == null || password.trim().isEmpty() || password.length() < 6 || password.length() > 50) {
                error.setPasswordLengthErr("Password must be between 6-50 Characters");
                valid = false;
